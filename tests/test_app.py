...
class TestTaskHandlers:
    ...
    async def test_get_task(self, created_task):
        async with httpx.AsyncClient() as client:
            response = await client.get(f'{self.url}/task_{created_task}', auth=self.auth)
            assert response.status_code == 200
    ...
    async def test_update_task(self, created_task):
        async with httpx.AsyncClient() as client:
            response = await client.patch(f'{self.url}/update/{created_task}', auth=self.auth, json=self.TEST_TASK)
            assert response.status_code == 200
            assert response.json().get('status') == 'successful'
    ...
    async def test_delete_task(self, created_task):
        ...
        async with httpx.AsyncClient() as client:
            response = await client.delete(f'{self.url}/delete/{created_task}')
            assert response.status_code == 200
            assert response.json().get('status') == 'successful'
